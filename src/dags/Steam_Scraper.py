...
with  DAG(
    ...
    def to_csv_file(list_games,csv_file):
        '''
    ...
    def to_mysql_insert_file(list_games,database,table):
        with open(Working_place + '/src/dags/SQLquery/Insert.sql','w') as f:
    ...
    Create = MySqlOperator(
        ...
        sql="/SQLquery/Create_Database_table.sql",
        database='Airflowdb'
    )
    ...
    email = EmailOperator(
        ...
        files=[ Working_place + '/output_example/game_prices.csv']
    )
    ...
    csv_file = Working_place + '/output_example/game_prices.csv'
    to_csv_file(game_list,csv_file) >> email
